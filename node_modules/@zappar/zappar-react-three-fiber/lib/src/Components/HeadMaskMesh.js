 ...
 */
const zapparHeadMaskMesh = react_1.forwardRef((props, ref) => {
    const { children, trackerGroup } = props;
    const headMaskMeshRef = react_1.default.useRef();
    const [faceMesh] = react_1.useState(new ZapparThree.HeadMaskMeshLoader().load());
    fiber_1.useFrame(() => {
    ...
    if (!faceMesh)
        return null;
    return (react_1.default.createElement("primitive", Object.assign({ object: faceMesh, ref: react_merge_refs_1.default([headMaskMeshRef, ref]) }, props), children));
});
