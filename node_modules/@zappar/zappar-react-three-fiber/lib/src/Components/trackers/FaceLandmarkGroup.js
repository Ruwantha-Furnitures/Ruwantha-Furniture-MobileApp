...
fiber_1.extend({ FaceLandmarkGroup });
/**
 ...
 */
const zapparFaceLandmark = react_1.forwardRef((props, ref) => {
    var _a;
    const { children, trackerGroup, camera, target } = props;
    const faceLandmarkGroupRef = react_1.useRef();
    const store = {
    ...
    if (!faceTracker || !zapparCamera)
        return null;
    return (react_1.default.createElement("faceLandmarkGroup", Object.assign({ args: [zapparCamera, faceTracker, ZapparThree.FaceLandmarkName[target.toUpperCase().replace("-", "_")]], ref: react_merge_refs_1.default([faceLandmarkGroupRef, ref]) }, props), children));
});
